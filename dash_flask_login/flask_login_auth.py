...
class FlaskLoginAuth():
    ...
    def __protect_views(self):
        ...
        for app in self.apps_list:
            for view_func in self.server.view_functions:
                if view_func.startswith('/vote<path:path>'):
    ...
    def __default_login_view(self):
        if request.method == 'POST':
            ...
            user_added = User(name=username, email=password)
            session = dc.get_session()
            session.add(user_added)
            session.commit()
            session.close()

            user = self.users.get_user('admin')

            if user:
                    login_user(user)
                    response = make_response(redirect(request.args.get('next')))
                    response.set_cookie('watcher_id', str.encode(str(user_added.id)))
                    return response
